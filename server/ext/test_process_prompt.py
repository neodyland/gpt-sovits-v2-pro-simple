import torch
from .kernel import process_prompt
...
if __name__ == "__main__":
    x = torch.rand(
        32, cfg["model"]["hidden_dim"], dtype=dtype, device=device
    ).contiguous()
    attn_mask = torch.triu(
        torch.ones(32, 32, device=device, dtype=torch.bool), diagonal=1
    )
    attn_mask_torch = attn_mask.unsqueeze(0).unsqueeze(0)
    attn_mask_kernel = attn_mask
    kernel = process_prompt.get_module(
        fused=True,
        model_attn_num_heads=cfg["model"]["head"],
        model_hidden_dim=cfg["model"]["hidden_dim"],
        model_mlp_hidden_dim=cfg["model"]["hidden_dim"] * 4,
        model_processing_seq_length=32,
    )
    ...
    forward = torch.compile(block.process_prompt_raw, fullgraph=True, dynamic=True)

    @torch.inference_mode()
    def run_pytorch():
        return forward(x.unsqueeze(0), attn_mask_torch, cache).float()
